...
spark.sql("""
--DROP DATABASE IF EXISTS demos CASCADE;
...
fileName = "noaa.csv" #dbutils.widgets.get("File Name")
...
spark.sql("""CREATE OR REPLACE TABLE Silver_AllSensors_Simple 
(
...
USING DELTA
LOCATION "/data/codydemos/silver_allsensors_simple_stream_2"
...
dbutils.fs.rm("dbfs:/FileStore/shared_uploads/cody.davis@databricks.com/IotDemoCheckpoints/BronzeToSilverSimple_2/", recurse=True)
...
spark.sql("""SELECT * FROM Silver_AllSensors_Simple""")

# COMMAND ----------

spark.sql("""OPTIMIZE Silver_AllSensors_Simple ZORDER BY (MeasurementDateTime) """)
